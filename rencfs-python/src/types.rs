...
use pyo3::prelude::*;
use rencfs::{
    Cipher as RustCipher, CreateFileAttr as RustCreateFileAttr, FileAttr as RustFileAttr,
    FileType as RustFileType, FsError, SetFileAttr as RustSetFileAttr,
};
...
impl From<RustFileType> for FileType {
    fn from(kind: RustFileType) -> Self {
        match kind {
            ...
            _ => FileType::RegularFile, // simplify for binding purposes
        }
    }
}
...
impl SetFileAttr {
    ...
    fn into_rust(self) -> RustSetFileAttr {
        let to_system_time =
            |opt: Option<u64>| opt.map(|secs| UNIX_EPOCH + std::time::Duration::from_secs(secs));
        RustSetFileAttr {
            size: self.size,
...
impl DirEntry {
    #[new]
    fn new(ino: u64, name: String, kind: FileType) -> Self {
        Self { ino, name, kind }
    }
}
...
impl DirEntryPlus {
    #[new]
    fn new(ino: u64, name: String, kind: FileType, attr: FileAttr) -> Self {
        Self {
            ino,
            name,
            kind,
...
impl From<PyFsError> for PyErr {
    fn from(err: PyFsError) -> PyErr {
        match err.0 {
            ...
            FsError::AlreadyExists => PyFileExistsError::new_err("File already exists"),
